...
export default function Insights({ language }: InsightsProps) {
  ...
  const totalSlides = Math.ceil(articles.length / cardsPerView)
    ...
    <section id="insights" className="py-16 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="relative px-12">
          ...
          <button
            onClick={handlePrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-white rounded-full p-2 shadow-md hover:shadow-lg hover:bg-gray-50 transition-all duration-200 group"
          ...
          <button
            onClick={handleNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-20 bg-white rounded-full p-2 shadow-md hover:shadow-lg hover:bg-gray-50 transition-all duration-200 group"
            aria-label="Next articles"
          >
            <ChevronRight className="w-5 h-5 text-gray-700 group-hover:text-blue-600" />
          </button>

          {/* 그리드 레이아웃 - 4개 카드가 한 눈에! */}
          <div className="overflow-hidden">
            ...
            >
              {articles.map((article, index) => (
                ...
                >
                  <div className="bg-gradient-to-br from-gray-50 to-white border border-gray-200 rounded-xl p-5 h-full hover:shadow-lg transition-shadow duration-300 group cursor-pointer">
                    {/* 카테고리 태그 */}
